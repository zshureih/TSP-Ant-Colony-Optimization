...
class Ant():
    def __init__(self, startCity, availableCities, pheromoneTrail, alpha, beta, getDistance, first_pass=False):
        # have ants run in seperate processes to speed up execution time
        self.process = Process(target=self.run, args=())
        self.first_pass = first_pass
        self.startCity = startCity
        self.availableCities = availableCities.copy()
        self.currentCity = startCity
        self.path = Array('i', range(len(availableCities) + 2))
        self.pathLength = Value('d', 0)
    ...
    def _update_path(self, nextCity):
        self.path[len(self.path) - len(self.availableCities) - 1] = nextCity

        if nextCity != self.startCity:
            self.availableCities.remove(nextCity)

        self.pathLength.value += self.distanceFunc(self.currentCity, nextCity)
...
class AntColony:
    ...
    def _init_ants(self, start):
        ...
        if self.first_pass:
            ants = [Ant(start, availableCities, self.pheromone_map,
				self.alpha, self.beta, self.getDistance, first_pass=True) for x in range(self.ant_count)]
            return ants
    ...
    def _populate_ant_updated_pheromone_map(self, ant):
        for i in range(len(ant.path) - 1):
            ...
            newPheromoneVal = self.pheromone_constant/ant.pathLength.value
    ...
    def run(self):
        for t in range(self.iterations):
            #start multi-processed ants, calls ant.run() in a new process
            for ant in self.ants:
                ant.process.start()
...
def main():
    ...
    outFile.truncate(0) #clear contents of file if not empty
    outFile.close()

    outFile = open(sys.argv[1] + ".tour", "a")

    #begin printing test case results
